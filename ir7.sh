...
# =================================================================
#      Advanced IPv6 Tunnel Manager (HUB & CLIENT) v4.4
#      Features: Summary Tables, Deep Clean, 7 Servers, Persistence
# =================================================================
...
#!/bin/bash
# Auto-generated by Tunnel Manager v4.4 (HUB Mode - 7 Servers)
...
EOF
    ...
    # --- Show Summary Table for HUB ---
    echo -e "\n${C_CYAN}======================================================================================${C_RESET}"
    echo -e "${C_CYAN}                              CONNECTED CLIENTS OVERVIEW                              ${C_RESET}"
    echo -e "${C_CYAN}======================================================================================${C_RESET}"
    printf " %-3s | %-15s | %-16s | %-25s | %-12s\n" "ID" "NAME" "PUBLIC IP" "LOCAL IPv6" "LOCAL IPv4"
    echo -e "-----+-----------------+------------------+---------------------------+-------------"
    ...
    for i in {0..6}; do
        ...
        printf " ${C_GREEN}%-3s${C_RESET} | ${C_YELLOW}%-15s${C_RESET} | ${C_MAGENTA}%-16s${C_RESET} | %-25s | %-12s\n" "[$s_num]" "${!name_var}" "${!ip_var}" "${v6_pre}::2" "${v4_pre}.2"
    done
    echo ""
}
...
function apply_client() {
    ...
    cat <<EOF > /etc/rc.local
#!/bin/bash
# Auto-generated by Tunnel Manager v4.4 (CLIENT Mode)
...
EOF
    ...
    # --- Show Summary Table for CLIENT ---
    echo -e "\n${C_CYAN}======================================================${C_RESET}"
    echo -e "${C_CYAN}                TUNNEL SPECIFICATIONS                 ${C_RESET}"
    echo -e "${C_CYAN}======================================================${C_RESET}"
    echo -e "   Client Name   : ${C_YELLOW}Client #$SERVER_NUM${C_RESET}"
    echo -e "   Public IPv4   : ${C_MAGENTA}$IP_IRAN${C_RESET}"
    echo -e "   Local IPv6    : ${C_GREEN}${V6_PRE}::2${C_RESET}"
    echo -e "   Local IPv4    : ${C_GREEN}${V4_PRE}.2${C_RESET}"
    echo -e "${C_CYAN}======================================================${C_RESET}\n"
}
...
while true; do
    clear
    echo -e "${C_CYAN}==============================================${C_RESET}"
    echo -e "${C_CYAN}    Advanced Tunnel Manager v4.4 (Summary)    ${C_RESET}"
    echo -e "${C_CYAN}==============================================${C_RESET}"
    echo ""
